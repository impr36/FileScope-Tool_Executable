...
class FileScope:
    ...
    def export_pdf(self):
        ...
        static_info = [
            ...
            ["Notes", Paragraph(f"({self.analysis_results['entropy']['LSB Check']}, Anomaly: {self.analysis_results['entropy']['Anomaly Detected']})", table_style)],
        ]
        ...
        if graph_buffer:
            story.append(Spacer(1, 6))
            story.append(Paragraph("Entropy Graph", styles['Heading3']))
        ...
        # Build the dynamic recommendation text
        risk_score = self.analysis_results['risk']['Score']
        risk_level = self.analysis_results['risk']['Level']

        recommendation_text = f"Risk Score: {risk_score}<br/>"
        if risk_level == "HIGH":
            recommendation_text += "Quarantine the file immediately. DO NOT run on production systems.<br/>"
        else:
            recommendation_text += "File appears safe but verify before execution.<br/>"

        # Combine preventive measures text and dynamic recommendation
        preventive_measures_text =  recommendation_text + "Update antivirus and apply patches.<br/>"

        remediation_info = [
            ["Suggested Actions", Paragraph("Quarantine the file immediately. DO NOT run on production systems." if risk_level == "HIGH" else "Verify before execution.", table_style)],
            ...
            ["Preventive Measures", Paragraph(preventive_measures_text, table_style)],
        ]
        ...
        story.append(Paragraph("<i>Generated by FileScope v2.0</i>", styles['Normal']))
        doc.build(story)
